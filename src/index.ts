// dsh-sidechat — node half.
//
// Codex /side-style side chats for the DSH web GUI:
//  - `POST /api/sidechat.start` — fork the source session at its latest
//    completed turn into a lineage child (same workspace/cwd), then append a
//    hidden-in-spirit boundary message that marks the inherited history as
//    reference-only for the child agent.
//  - a `sidechat` session projection marks fork children (wire value
//    `{ parentId }`) so the client can render side-chat UI without any
//    header schema change (header `origin` is a closed union).
//  - `sidechat_check_parent` tool: the side-chat agent can pull the parent
//    session's live surface on demand (continuous attention on progress).
...
export const inject = ["webServer", "apiProxy", "sessions", "agents", "sessionQuery", "tools"];
...
*/
export const BOUNDARY_PROMPT = `【侧聊边界 · Side conversation boundary】
...
function boundarySource(parentId) {
	return { kind: "plugin", plugin: "side-chat", form: "boundary", parentId };
}
...
function isBoundaryMessage(event) {
	return event.type === "user/message"
		...
		&& event.data?.source?.plugin === "side-chat"
		&& event.data?.source?.form === "boundary";
}
...
async function startSideChat(ctx, sessionId, atSeq) {
	...
	const response = await api.sessions.fork({
		rpcId: randomUUID(),
		payload: { sessionId, ...atSeq === void 0 ? {} : { atSeq } }
	});
	...
	if (child !== void 0) {
		child.append("user/message", createUserMessage({
			content: [{ type: "text", text: BOUNDARY_PROMPT }],
		...
		try {
			...
			try {
				if (envelope.method === "sidechat.start") {
					...
					respond({ ok: true, value: { sessionId: childId } });
				} else {
					respond({ ok: false, error: { code: "method-not-found", message: `unknown sidechat method "${envelope.method}"`, details: {} } });
				}
			} catch (error) {
				respond({
					ok: false,
					error: { code: "sidechat-error", message: error instanceof Error ? error.message : String(error), details: {} }
				});
			}
		} catch (error) {
			try {
...
function renderSurface(parentId, surface, limit = 6000) {
	const blocks = [];
	let bytes = 0;
	const push = (text) => {
...
export function apply(ctx) {
	// 1. Projection: mark side-chat children for every client (summaries +
	//    session/projection push frames). Registration is optional-safe: the
	//    seam is composed in the shipped web profile, absent elsewhere.
	...
	// 2. HTTP routes: exact-path registration beats the generic /api prefix
	//    (longest-prefix-wins does not apply to dot-separated method paths).
	//    Each request still rides the browser trust fence replicated above.
	for (const method of METHODS) {
		ctx.effect(() => ctx.webServer.register({
			kind: "exact",
			path: `${API_PREFIX}.${method.slice("sidechat.".length)}`,
			handler: routeHandler(ctx)
		}), `dsh-sidechat: ${method} route`);
	}

	// 3. Model tool: the side-chat agent can read the parent's live surface.
	ctx.tools.register(defineTool({
		...
		presentCall: () => ({ card: "generic", title: "Check parent session surface", kind: "read" })
	}));
}
